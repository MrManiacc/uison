...
public class Constraints {
    ...
    public static class StickyConstraint implements Constraint {
        public enum FACE {
            LEFT("LEFT"), RIGHT("RIGHT"), TOP("TOP"), BOTTOM("BOTTOM"), NONE("OTHER");
            ...
            public static FACE parse(String value) {
                for (FACE face : FACE.values()) {
                    if (face.value.toLowerCase().equals(value.toLowerCase()))
                        return face;
                }
                return LEFT;
            }
        ...
        public void setRelativeConstraintID(String id) {
            this.relativeConstraintID = Optional.of(id);
        }


        public boolean hasRelativeConstraint() {
            if (relativeConstraint.isEmpty() && relativeConstraintID.isPresent()) {
                if (UIMaster.hasComponent(relativeConstraintID.get())) {
                    UIComponent relative = UIMaster.component(relativeConstraintID.get()).getComponent();
                    if (relative != null) {
                        if (relative instanceof UIConstraint) {
                            this.relativeConstraint = Optional.of((UIConstraint) relative);
                        } else {
        ...
        public UIConstraint getRelativeConstraint() {
            if (relativeConstraint.isEmpty() && relativeConstraintID.isPresent() && UIMaster.hasComponent(relativeConstraintID.get())) {
                if (UIMaster.hasComponent(relativeConstraintID.get())) {
                    UIComponent relative = UIMaster.component(relativeConstraintID.get()).getComponent();
                    if (relative != null) {
                        if (relative instanceof UIConstraint) {
    ...
    public static class RelativeConstraint implements Constraint {
        ...
        private Optional<String> relativeConstraintID = Optional.empty();
        ...
        public RelativeConstraint setRelativeConstraint(UIComponent parent) {
    ...
    public static class CenterConstraint implements Constraint {
        ...
        public CenterConstraint(double offset) {
            this.offset = offset;
        }
